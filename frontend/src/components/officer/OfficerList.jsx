...
const OfficerList = () => {
  ...
  // Function to update current user in localStorage
  const updateCurrentUserInStorage = useCallback((updatedOfficerData) => {
    try {
      const userData = localStorage.getItem("user");
      if (userData) {
        const currentUser = JSON.parse(userData);

        // Check if this is the current user being updated
        if (currentUser._id === updatedOfficerData._id) {
          // Update the current user's data in localStorage
          const updatedUser = {
            ...currentUser,
            name: updatedOfficerData.name,
            phone: updatedOfficerData.phone,
            designation: updatedOfficerData.designation,
            department: updatedOfficerData.department,
            unit: updatedOfficerData.unit || currentUser.unit,
            bpNumber: updatedOfficerData.bpNumber,
            status: updatedOfficerData.status,
          };
          ...
          console.log(
            "Current user updated in localStorage and event dispatched"
          );
        }
      }
    } catch (error) {
  ...
  const updateOfficerStatus = async (officerId, newStatus) => {
    try {
      ...
      // Check if this is the current user being updated
      if (officerId === currentUserId) {
        updateCurrentUserInStorage(response.data.officer);
      }
                                ...
                                <div>
                                  ...
                                  <div className="relative">
                                    <input
                                      ...
                                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:border-blue-600 focus:outline-none pr-10"
                                    />
                                    <button
                                      type="button"
                                      onClick={() =>
